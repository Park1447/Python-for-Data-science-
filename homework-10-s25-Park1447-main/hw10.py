...
def conf_matrix(y_pred, y_true, num_class):
    ...
    M = np.zeros((num_class, num_class), dtype=int)
    
    y_true = np.array(y_true, dtype=int)
    y_pred = np.array(y_pred, dtype=int)

    for i in range(len(y_true)):
        actual_array = y_true[i]
        predicted_array = y_pred[i]
        M[actual_array][predicted_array] += 1
...
def get_model_results(model_name, params, train_data, train_labels, test_data, test_labels, num_class):
    ...
    acc = metrics.accuracy_score(test_labels, predicted_test)
    # 5. Calculate the confusion matrix by using the completed the function above 
    conf_mat = conf_matrix(predicted_test, test_labels, num_class)
    # 6. Compute the AUROC score. You may use metrics.roc_auc_score(...)
    class_prob = model.predict_proba(test_data)
    auc_scor = metrics.roc_auc_score(test_labels, class_prob, multi_class='ovr')
