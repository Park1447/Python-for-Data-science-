...
def build_doc_word_matrix(doclist, n, normalize=False):
    ...
    cleaned_doc = []
    for word in doclist:
        cleaned_doc.append(read_and_clean_doc(word))

    # 2. Create and use ngram lists to build the doc word matrix
    num_ngrams = []
    total_ngrams = []
    for word in cleaned_doc:
        each_gram = get_ngrams(word, n)
        num_ngrams.append(each_gram)
        total_ngrams += each_gram

    ngramlist = sorted(list(set(total_ngrams)))
    ngram_index = {}
    for i in range(len(ngramlist)):
        ngram_index[ngramlist[i]] = i
    docword = np.zeros((len(doclist), len(ngramlist)))
    for doc_idx in range(len(num_ngrams)):
        for ng in num_ngrams[doc_idx]:
            col_idx = ngram_index[ng]
            docword[doc_idx][col_idx] += 1
    if normalize:
        docword_copy = docword.copy()
        row_sums = docword_copy.sum(axis=1, keepdims=True)
        docword = docword_copy / row_sums
        docword = np.round(docword, 4)
...
def build_tf_matrix(docword):
    ...
    # TODO: fill in
    docword_copy = docword.copy()
    tf = docword_copy / docword_copy.sum(axis=1, keepdims=True)
...
def build_idf_matrix(docword):
    ...
    # TODO: fill in
    docword_copy = docword.copy()
    num_docs = docword_copy.shape[0]
    df = np.count_nonzero(docword_copy > 0, axis=0)
    idf_values = np.log10(num_docs / df)
    idf = idf_values[np.newaxis, :]
