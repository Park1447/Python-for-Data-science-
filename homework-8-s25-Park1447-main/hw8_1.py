...
def get_dict(filename, n):
    ...
    store_ngrams = []
    for line in lines:
         store_ngrams.extend(get_ngrams(line,n))
    k = set(store_ngrams)

    # 2. use the set to initialize a dict
    #   fill in
    ngram_dict = dict.fromkeys(k, 0)
    # 3. update the values of the dict
    #   fill in
    for ngram in store_ngrams:
        ngram_dict[ngram] += 1
    return ngram_dict
...
def top_N_common(filename, N, n, threshold=0):
    ...
    # 2. sort the dict
    # fill in
    threshold_dict = {k: v for k, v in ngram_dict.items() if v >= threshold}
    sorted_items = sorted(threshold_dict.items(), key=lambda item: -item[1])
...
def compare_langs(test_file, langFiles, N, n=3):
    ...
    # fill in
    mystery_topN = set(top_N_common(test_file, N, n).keys())
    # 2. cardinalities of intersections, use 'intersection()' and 'top_N_common' function
    # fill in
    temp = 0
    for find_lang in langFiles:
        find_langN = set(top_N_common(find_lang,N,n).keys())
        count = len(mystery_topN.intersection(find_langN))
        if count > temp:
            temp = count
            lang_match = find_lang
