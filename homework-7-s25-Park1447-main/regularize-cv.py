...
def main():
    ...
    '''
    plt.plot(lmbda, MSE, marker = 'o', linestyle='-')  # <<< fill in here
    plt.xscale('log')
    ...
    fill in your code below 
    '''
    print("The model coefficients are ", model_best.coef_)
    print("The model intercept are", model_best.intercept_)
    # Part 8
    # Load GOOG.csv 
    ...
    X_goog = np.array(df_goog.drop(["Prediction"], axis=1))  # <<< fill in here (similar to step 6)
    y = np.array(df_goog["Prediction"])  # <<< fill in here (similar to step 7)

    # normalize X similar to X_test
    X_test = normalize_test(X_goog, trn_mean, trn_std)
    y_hat = model_best.predict(X_test)  # <<< fill in here (use your best model from Part 7)
    ...
    plt.plot(y_hat, label='Predicted', color='orange'),
    plt.xlabel('Days')
