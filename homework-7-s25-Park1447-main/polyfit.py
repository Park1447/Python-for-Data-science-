...
def feature_matrix(x, d):
    ...
    fill in your code here
    '''
    X = [[xi**j for j in range(d, -1, -1)] for xi in x]
...
if __name__ == "__main__":
    ...
    for params in paramFits:
        ...
        Fill in your code here
        '''
        y_pred_test = [sum(params[j] * (xi ** (len(params) - 1 - j)) for j in range(len(params))) for xi in x]
        plt.plot(x, y_pred_test, color=colors[index], label=f'd = {degrees[index2]}')
        index += 1
        index2 += 1
    ...
    '''
    x_value = 4
    best_params = paramFits[2]
    y_pred_test = sum(best_params[j] * (x_value ** (len(best_params) - 1 - j)) for j in range(len(best_params)))
    print("The predicted y_value is:", y_pred_test)
