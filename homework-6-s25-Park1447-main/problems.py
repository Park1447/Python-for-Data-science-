...
def problem1(searchstring):
    ...
    :return: string
    """
    x = re.compile(r'^[a-zA-Z]{1,10}\.[1-7][0-9]{2}[a-zA-Z]*@(shield\.gov|starkindustries\.com)$')
    if x.fullmatch(searchstring):
        return "valid"
    else:
        return "invalid"
...
def problem2(searchstring):
    ...
    :return: tuple
    """
    p = re.compile(r'([A-Z][a-z]+(?: [A-Z][a-z]+)?) wrote ((?:[A-Z0-9][a-z0-9]* ?){1,3}|books)')
    matches = p.search(searchstring)
    if matches:
        author, title = matches.group(1), matches.group(2)
...
def problem3(searchstring):
    ...
    :return: string
    """
    pattern = re.compile(r'\b([A-Z][a-z]+) (Boy|Girl|boy|girl)\b')
    replacements = {
        'Boy': 'Man',
        'Girl': 'Woman',
        'boy': 'man',
    ...
    result = re.sub(pattern, lambda s: s.group(1) + " " + replacements[s.group(2)], searchstring)  
