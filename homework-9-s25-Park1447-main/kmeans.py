...
def kmeans(point_data, cluster_data):
    ...
    while moved_cluster:
        moved_cluster = False
        point_cluster = {pc.center: pc for pc in clusters}
        for po in points:
            closest_cluster = po.closest(list(point_cluster.keys()))
            close_cluster = point_cluster[closest_cluster]
            if po.moveToCluster(close_cluster):
                moved_cluster = True
        for f_cluster in clusters:
            f_cluster.updateCenter()
